...
def get_nusc_val_token(nusc):
    # [修改逻辑] 直接提取当前数据集的所有场景名和所有样本 Token
    # 适配 mini, trainval, test 全版本
    val_scene_ids = [s['name'] for s in nusc.scene]
    sample_val_tokens = [s['token'] for s in nusc.sample]
    
    print("Total scenes: {}".format(len(val_scene_ids)))
    print("Total samples to process: {}".format(len(sample_val_tokens)))
...
if __name__ == '__main__':
    ...
    # cp to ./data/nuscenes/
    final_path = os.path.join(opts.data_path, '{}_{}Hz_full_trainval'.format(opts.ann_strategy, opts.ann_frequency))
