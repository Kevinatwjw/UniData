...
def get_nusc_val_scene_token(nusc):
    # [修改逻辑] 不再通过 splits 过滤，而是直接获取当前数据集中的所有场景 Token
    # 这样可以自动适配 mini (10个), trainval (850个) 和 test (150个)
    val_scene_tokens = [scene['token'] for scene in nusc.scene]
    print(f"Total scenes to process: {len(val_scene_tokens)}")
...
if __name__ == '__main__':
    ...
    # save
    # 命名规则：20Hz_lidar_infos_{train/val}.pkl
    info_val_path = os.path.join('./out/lidar_20Hz_full', '20Hz_lidar_full_infos_val.pkl')
