...
def create_nuscenes_infos(root_path,
    ...
    # ================= 【修改部分 START】 =================
    # 策略：分离 "逻辑版本" 和 "加载版本"
    # 如果 root_path 下找不到名为 version 的子文件夹，但直接有 json 文件，
    # 则告诉 NuScenes API 去读当前目录 ('.')，但在逻辑上依然视为 version (v1.0-mini)。
    
    expected_subdir = os.path.join(root_path, version)
    if not os.path.exists(expected_subdir):
        print(f"Warning: Subdirectory '{version}' not found in {root_path}.")
        print(f"Assuming JSON tables are located directly in root_path.")
        # 使用 '.' 作为加载版本，告诉 API 直接读根目录
        load_version = '.' 
    else:
        # 正常结构，加载版本 = 逻辑版本
        load_version = version

    # 初始化 NuScenes (使用 load_version)
    nusc = NuScenes(version=load_version, dataroot=root_path, verbose=True)
    # ================= 【修改部分 END】 =================
    ...
    if test:
        ...
        # 2. 转换为 Scene 格式并保存
        # print('Converting TEST infos to scene format...')
        # test_scene_infos = transform_to_scene_format(train_nusc_infos)
        # data_test_scene = dict(
        #     infos=test_scene_infos, 
        #     metadata=metadata, 
        #     scene_tokens=scene_tokens
        # )
        # scene_path = osp.join(out_path, '{}_interp_12Hz_infos_test_scene.pkl'.format(info_prefix))
        # mmcv.dump(data_test_scene, scene_path)
        ...
        # 2. 转换为 Scene 格式并保存
        # print('Converting TRAIN infos to scene format...')
        # train_scene_infos = transform_to_scene_format(train_nusc_infos)
        # data_train_scene = dict(
        #     infos=train_scene_infos, 
        #     metadata=metadata, 
        #     scene_tokens=scene_tokens
        # )
        # train_scene_path = osp.join(out_path, '{}_interp_12Hz_infos_train_scene.pkl'.format(info_prefix))
        ...
        # # [MODIFIED] 注释掉 Scene 格式转换
        # # 2. 转换为 Scene 格式并保存
        # print('Converting VAL infos to scene format...')
        # val_scene_infos = transform_to_scene_format(val_nusc_infos)
        # data_val_scene = dict(
        #     infos=val_scene_infos, 
        #     metadata=metadata, 
        #     scene_tokens=scene_tokens
        # )
        # val_scene_path = osp.join(out_path, '{}_interp_12Hz_infos_val_scene.pkl'.format(info_prefix))
        # mmcv.dump(data_val_scene, val_scene_path)

    print('[Success] All 12Hz files (List format) generated.')
    # ----------------------------------------------------------------------------
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        # [MODIFIED] 注释掉 can_bus，官方 12Hz pkl 不包含此字段
        ...
        info = {
            ...
            # [MODIFIED] 目标 12Hz 格式不包含链表指针和 canbus
            # 'prev': sample['prev'],
            # 'next': sample['next'],
            # 'can_bus': can_bus,
            ...
            # [MODIFIED] 目标 list 元素中不包含 scene_token (它在顶层 list 中)
        ...
        if not test:
            ...
            # =============== 【修改部分 START: 提取 Visibility】 ===============
            visibility = _extract_visibility(annotations)
            # ===============================================================
            ...
            # [MODIFIED] 从这里开始，注释掉所有 unidata 的预测和规划逻辑
            ...
            # # =============== 【最终修正方案】 ===============
            # # 依据：脚本分析证实官方文件中的 pose_mode 与 gt_ego_fut_cmd 完全一致
            # # 目的：复现官方数据格式，传入导航指令
            # info['pose_mode'] = command.astype(np.float32)
            # # ==============================================
...
python tools/data_converter/unidata_nuscenes_12Hz_converter.py nuscenes \
    --root-path ./data/nuscenes/advanced_12Hz_mini \
    --canbus ./data/nuscenes \
