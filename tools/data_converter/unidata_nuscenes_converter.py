...
def create_nuscenes_infos(root_path,
    ...
    if test:
        ...
        # ----------------- 【新增】追加保存 Scene 格式 (Test) -----------------
        print('Converting TEST infos to scene format...')
        test_scene_infos = transform_to_scene_format(train_nusc_infos)
        # 创建一个新的字典，避免影响原来的 data
        data_scene = dict(infos=test_scene_infos, metadata=metadata)
        scene_info_path = osp.join(out_path, '{}_infos_temporal_test_scene.pkl'.format(info_prefix))
        mmcv.dump(data_scene, scene_info_path)
        # ---------------------------------------------------------------------
        ...
        # ----------------- 【新增】追加保存 Scene 格式 (Train & Val) -----------------
        print('\n[unidata Extra] Start converting to Scene Format...')

        # (1) 转换并保存 TRAIN_SCENE
        print('Converting TRAIN infos to scene format...')
        train_scene_infos = transform_to_scene_format(train_nusc_infos)
        data_train_scene = dict(infos=train_scene_infos, metadata=metadata)
        scene_train_path = osp.join(out_path, '{}_infos_temporal_train_scene.pkl'.format(info_prefix))
        mmcv.dump(data_train_scene, scene_train_path)

        # (2) 转换并保存 VAL_SCENE
        print('Converting VAL infos to scene format...')
        val_scene_infos = transform_to_scene_format(val_nusc_infos)
        data_val_scene = dict(infos=val_scene_infos, metadata=metadata)
        scene_val_path = osp.join(out_path, '{}_infos_temporal_val_scene.pkl'.format(info_prefix))
        mmcv.dump(data_val_scene, scene_val_path)
        
        print('[unidata Extra] All scene format files saved.')
        # ----------------------------------------------------------------------------
